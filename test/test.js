...
describe('envCheck', function() {
    ...
    describe('checkAll', function() {
		...
		it('envArr should be [ "ENV1", "env2", "env5", "env6" ]', async function() {
		    const envArr = await chkPromise.then(envchk.genAST)
			assert.equal("ENV1", envArr[0])
			assert.equal("env2", envArr[1])
			assert.equal("env5", envArr[2])
			assert.equal("env6", envArr[3])
		})
    })
})
